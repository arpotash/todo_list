...
class TestAuthUserViewSet(TestCase):
    ...
    def test_update_auth_user(self):
        request = self.client.put(f'/api/users/{self.user.id}/',
                                  {'username': 'username1',
    ...
    def test_get_details_user(self):
        ...
        request = self.client.get(f'/api/users/{user.id}/')
        self.assertEqual(request.status_code, status.HTTP_200_OK)
...
class TestGuestViewSet(TestCase):
    ...
    def test_update_other_roles(self):
        response = self.client.patch(f'/api/users/{self.user.id}/', {'email': 'new_email@mail.ru', 'age': 22})
